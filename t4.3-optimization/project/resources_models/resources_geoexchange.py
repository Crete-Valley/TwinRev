...
class GeoExchange(Resource):
    ...
    def __init__(
        ...
        GeoExchange.resource_count += 1
        self.max_power = max_power
        self.type = "GeoExchange"
    ...
    def get_variables(self, model) -> None:
        if getattr(model, "P_GE_E", False):
            return
        self._create_variables_energy(model, GeoExchange.resource_count)
        self._create_reserves_variables_reserve(model, GeoExchange.resource_count)
    ...
    def _create_variables_energy(self, model, resources_count) -> None:
        ...
        m = model
        m.P_GE_E = Var(arange(resources_count), arange(self.hours), domain=NonNegativeReals)
        m.P_GE_H = Var(arange(resources_count), arange(self.hours), domain=NonNegativeReals)
        m.P_load_GE = Var(arange(resources_count), arange(self.hours), domain=NonNegativeReals)
    ...
    def _create_reserves_variables_reserve(self, model, resources_count) -> None:
        ...
        if (self.system_reserves_participation or self.system_flexibility_participation) and self.resource_reserves_participation:
            ...
            m.U_GE = Var(arange(resources_count), arange(self.hours), domain=NonNegativeReals)
            m.D_GE = Var(arange(resources_count), arange(self.hours), domain=NonNegativeReals)

    def _create_constraint_energy(self, model) -> None:
        m = model
        for t in range(0, self.hours):
            m.c1.add(m.P_GE_E[self.resource_id_model, t] <= self.max_power)
            m.c1.add(m.P_GE_H[self.resource_id_model, t] == self.efficiency_heat * m.P_GE_E[self.resource_id_model, t])
    ...
    def _create_constraint_reserves(self, model) -> None:
        if (self.system_reserves_participation or self.system_flexibility_participation) and self.resource_reserves_participation:
            ...
            m = model
            for t in range(0, self.hours):
                m.c1.add(m.U_GE[self.resource_id_model, t] <= self.flexibility_range * m.P_GE_E[self.resource_id_model, t])
                m.c1.add(m.U_GE[self.resource_id_model, t] + m.P_GE_E[self.resource_id_model, t] <= self.max_power)
                m.c1.add(m.D_GE[self.resource_id_model, t] <= self.flexibility_range * m.P_GE_E[self.resource_id_model, t])

    def get_electricity_output(self, model, hour):
        return model.P_GE_E[self.resource_id_model, hour]

    def get_thermal_output(self, model, hour):
        return -model.P_GE_H[self.resource_id_model, hour] * self.efficiency_heat

    def get_upward_reserve_output(self, model, hour):
        if (self.system_reserves_participation or self.system_flexibility_participation) and self.resource_reserves_participation:

            return model.U_GE[self.resource_id_model, hour]
        else:
            return 0
    ...
    def get_downward_reserve_output(self, model, hour):
        if (self.system_reserves_participation or self.system_flexibility_participation) and self.resource_reserves_participation:
            ...
            return model.D_GE[self.resource_id_model, hour]
        else:
            return 0
    ...
    def save_results(self, model, writer):
        ...
        df = pd.DataFrame({
            ...
            "Electricity Input (kW)": [m.P_GE_E[self.resource_id_model, t]() for t in range(self.hours)],
            "Heat Output (kW)": [m.P_GE_H[self.resource_id_model, t]() for t in range(self.hours)],
            "Upward Reserve (kW)": [
                m.U_GE[
        ...
        if not df.empty:
            df.to_excel(writer, sheet_name=f'GeoExchange_{self.resource_id_original}', index=False)
