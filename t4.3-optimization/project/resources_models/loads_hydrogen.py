...
class HydrogenLoad(Loads):
    resource_count = 0
    def __init__(
        self,
        resource_id_original: int,
        resource_id_model: int,
        hydrogen_load_profile: list,
        hours: int,
        units: str = "kW",
    ):
        super().__init__(
        ...
        HydrogenLoad.resource_count += 1
        self.hydrogen_load_profile = hydrogen_load_profile
        self.hours = hours
        self.units = units
        self.type = "HydrogenLoad"


    def __repr2__(self, abbreviated: int = 1) -> str:
            return repr(
                f"Type: {self.type} |"
                f"Load profile {self.units}: {self.hydrogen_load_profile} |"
    ...
    def get_variables(self, model) -> None:
        if getattr(model, "P_load_hydrogen", False):
            return
        self._create_variables_energy(model, HydrogenLoad.resource_count)
    ...
    def _create_variables_energy(self, model, resources_count) -> None:
        ...
        m = model
        m.P_load_hydrogen = Var(arange(resources_count), arange(self.hours), domain=NonNegativeReals)


    def _create_constraint_energy(self, model) -> None:
        m = model
        for t in range(0, self.hours):
            m.c1.add(m.P_load_hydrogen[self.resource_id_model, t] == self.hydrogen_load_profile[t])
    ...
    def save_results(self, model, writer):
        ...
        df = pd.DataFrame({
            ...
            "Hydrogen Load (kW)": [m.P_load_hydrogen[self.resource_id_model, t].value for t in range(self.hours)],
        })
        if not df.empty:
            df.to_excel(writer, sheet_name=f'LoadHydrogen_{self.resource_id_original}', index=False)
